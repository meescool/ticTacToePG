...
class Button():
    ...
    def hover(self,scr,fr):
        ...
        if x >= self.x and x <= self.x +self.w and y >= self.y and y <= self.y + self.h:
            pygame.draw.rect(scr,self.colors[3],self.box,self.width,self.corners[0],self.corners[1],self.corners[2],self.corners[3])
            if fr <= 50:
                u.write_text(scr,self.text,a.fonts['btn'],self.colors[1],self.x+(self.w/2),self.y+(self.h/2))
            else:  
                u.write_text(scr,self.text,a.fonts['btn'],self.colors[2],self.x+(self.w/2),self.y+(self.h/2))
